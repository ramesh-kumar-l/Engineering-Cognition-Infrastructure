...
- ✅ **Phase E — Observability & guidance polish**: Observability screen — live `/metrics` rollup (`parseMetrics` over Prometheus text: requests, success rate (SLO-1), avg latency, auth denials, ingest ops), `/healthz`+`/readyz` probe cards, SLO panel (5 SLOs from `slos.md`), and docs/dashboard deep-links (Swagger/ReDoc/OpenAPI/Grafana/raw metrics). Reusable `NextStep` nudge closes the loop; Overview stages all marked live. New `apiText()` raw-text client helper. No backend change.

**Status:** 17/17 Vitest pass; `npm run build` clean (295 modules). All five FE phases (A–E) verified live against the running API. All FE files under the 300-line cap. **Env note:** Compress/Embed/Search-embedding and retrospective lesson-extraction need an LLM provider (Ollama not installed in this dev env → those paths 5xx / yield 0 lessons; UI fails closed). Ingest, Execution, manual Reflection capture, and Observability are LLM-free.
